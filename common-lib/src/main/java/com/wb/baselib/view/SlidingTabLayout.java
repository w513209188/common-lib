...
import com.wb.baselib.interfaces.OnTabSelectListener;
import com.wb.baselib.phone.PhoneUtils;
import com.wb.baselib.utils.UnreadMsgUtils;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    //    private float mBigTextSize;//大字体
    ...
    private boolean mIndicatorIsTop;
    private boolean mTabIsCenter;
    private Bitmap mCustomImageIndicator;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        //线是否靠上一点
        mIndicatorIsTop = ta.getBoolean(R.styleable.SlidingTabLayout_tl_indicator_is_top, false);
        mTabIsCenter = ta.getBoolean(R.styleable.SlidingTabLayout_tl_tab_is_center, false);
        Drawable custimdrawable = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_is_custom_image);
        if (custimdrawable != null) {
            BitmapDrawable bitmapDrawable = (BitmapDrawable) custimdrawable;
            mCustomImageIndicator = bitmapDrawable.getBitmap();
        }
...
//        mBigTextSize = sp2px(16);
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.public_layout_tab, null);
            CharSequence pageTitle = mTitles == null ? mViewPager.getAdapter().getPageTitle(i) : mTitles.get(i);
            if (mIndicatorStyle == STYLE_CUSTOM) {
                tabView.setPadding(dp2px(8), 0, dp2px(8), 0);
            }
            addTab(i, pageTitle.toString().trim(), tabView);
        }
    ...
    public void addNewTab(String title) {
        View tabView = View.inflate(mContext, R.layout.public_layout_tab, null);
        if (mTitles != null) {
            mTitles.add(title);
        }
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        if (mCustomImageIndicator == null) {
            setTabViewClickListen(tabView);
        }
...
//                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == mCurrentTab ? mBigTextSize : mTextsize);
//                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == 0 ? mTextsize + 8 : mTextsize);
                ...
                if (mIndicatorStyle == STYLE_CUSTOM) {
                    ...
                    tv_tab_title.setBackgroundResource(i == mCurrentTab ?
                            R.drawable.video_tags_border_pressed : R.drawable.video_tags_border_normal);
                    int top = 3;
                    if (mScale >= 3) {
                        top = 4;
                    }
                    tv_tab_title.setPadding(dp2px(8), dp2px(top), dp2px(8), dp2px(3));
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, sp2px(12));
                }
                ...
                tv_tab_title.setTypeface(i == 0 ? Typeface.defaultFromStyle(Typeface.BOLD) : Typeface.defaultFromStyle(Typeface.NORMAL));
            }
        }
    }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
//                tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, isSelect ? mTextsize + 3 : mTextsize);
                ...
                if (mIndicatorStyle == STYLE_CUSTOM) {
                    tab_title.setBackgroundResource(isSelect ?
                            R.drawable.video_tags_border_pressed : R.drawable.video_tags_border_normal);
                    int top = 3;
                    if (mScale >= 3) {
                        top = 4;
                    }
                    tab_title.setPadding(dp2px(8), dp2px(top), dp2px(8), dp2px(3));
                    tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, sp2px(12));
                }
//                tab_title.getPaint().setFakeBoldText(isSelect ? true : false);
                tab_title.setTypeface(isSelect ? Typeface.defaultFromStyle(Typeface.BOLD) : Typeface.defaultFromStyle(Typeface.NORMAL));
            }
        }
    }
...
//                mIndicatorDrawable.setStroke(UIUtils.dip2px(1),R.color.c_white);
            ...
            if (mIndicatorHeight > 0) {
                ...
                if (mIndicatorGravity == Gravity.BOTTOM) {
                    ...
                    if (mIndicatorIsTop) {
                        mIndicatorDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
                                height - (int) mIndicatorHeight - (int) mIndicatorMarginBottom - PhoneUtils.newInstance().dip2px(AppUtils.getContext(),9),
                                paddingLeft + mIndicatorRect.right - (int) mIndicatorMarginRight,
                                height - (int) mIndicatorMarginBottom - PhoneUtils.newInstance().dip2px(AppUtils.getContext(),9));
                    } else {
                        mIndicatorDrawable.setBounds(paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
                ...
                if (mCustomImageIndicator != null) {
                    canvas.drawBitmap(mCustomImageIndicator, paddingLeft + (int) mIndicatorMarginLeft + mIndicatorRect.left,
                            height - (int) mIndicatorHeight - (int) mIndicatorMarginBottom, null);
                } else {
                    mIndicatorDrawable.setCornerRadius(mIndicatorCornerRadius);
                    mIndicatorDrawable.draw(canvas);
                }
    ...
    public void setTextSelectColor(int textSelectColor) {
        if (this.mTextSelectColor != textSelectColor) {
    ...
    public void setTextUnselectColor(int textUnselectColor) {
        if (this.mTextUnselectColor != textUnselectColor) {
    ...
    public void setmCustomImageIndicator(Drawable drawable) {
        if (drawable != null) {
    ...
    public void setTabViewClickListen(View tabView) {
        tabView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                try {
                    if (mTabsContainer == null || mViewPager == null) {
                        return;
                    }
            ...
            case MotionEvent.ACTION_UP:
                ...
                try {
                    ...
                    for (int i = 0; i < mTabCount; i++) {
                        ...
                        TextView tv_tab_title = (TextView) v.findViewById(R.id.tv_tab_title);
                        if (tv_tab_title != null) {
                            RectF rect = calcViewScreenLocation(tv_tab_title);
                            if (rect != null) {
                                boolean isInViewRect = rect.contains(x, y);
                                if (isInViewRect && mCustomImageIndicator != null) {
                                    try {
                                        if (mTabsContainer == null || mViewPager == null) {
                                            return false;
                                        }
    ...
    public RectF calcViewScreenLocation(View view) {
        if (null != view) {
            ...
            return new RectF(location[0], location2[1], location[0] + view.getWidth(),
                    location2[1] + getHeight());
        }
        return null;
    }
}
